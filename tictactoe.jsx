const Board = () => {
  ...
  let player_name = ['O', 'X']; //CMB
  let player_symbol = player_name[player];
  ...
  const takeTurn = (id) => {
    ...
    setPlayer((player + 1) % 2); // get next player (0 o 1)
    return player;
  };
  ...
  return (
    <div className="game-board">
      ...
      <div id="info">
        ...
        */}
        <h1 id="turn">Next Player: Player {player_symbol}</h1>
        <h1>{status}</h1>
      </div>
    </div>
  );
...
const Square = ({ takeTurn, id }) => {
  ...
  return (
    ...
    >         
    <h1 className = {palet[tik]} >{mark[tik]}</h1>      
